...
const typeNum = (arr) => {
  // Solution code here...
  let newArry = arr.filter( value => (typeof(value)=== 'number')); // to check the type of the value in each index
  return newArry;                                                  // sources of info https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/typeof
};
...
const containsAnd = (arr) => {
  // Solution code here...
  let newArry = arr.filter(value => value.includes('and')); //to check if the value incldes 'and' in it
  return newArry;                                          //sources of info https://www.w3schools.com/jsref/jsref_includes.asp
};
...
let newArry = arr.filter(value => /[aeiou]/.test(value)); //aeiou are the vowels
return newArry;
