...
function transformToLis(obj){
  ...
  // let newArry =[];
  // Object.entries(obj).forEach(value => newArry.push(`<li>${value.join(': ')}</li>`))
  // return newArry;
...
const addPurchases = (arr) => {
  // Solution code here...
  const addPur = arr.reduce((acc, value , idx)=>
  {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const countElement = arr.reduce((acc , value , idx)=> {
    return acc+1;
  },0)
  return countElement;
};
...
const reversedString = (str) => {
  // Solution code here...
 let splitedStr = str.split(''); //[c,o,d,e, ]

  const newRevStr = splitedStr.reduce((acc,value,idx)=>{
    return value+acc;
  },'')
  return newRevStr;
};
