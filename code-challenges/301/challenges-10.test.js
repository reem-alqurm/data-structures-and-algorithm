...
const getNames = (arr) => {
  // Solution code here...
  return arr.map((value,inx)=>value.name.split('').reverse().join('')); // i tried to use reduce but it didnt woked.
    // 
  //  
...
const divisibleByFiveTwoToThePower = (input) => { // to access the two dimintion array we need to use two map method
  // Solution code here...
  let newArray = input.map(element => element.filter (value => typeof(value) === 'number' && !(value % 5))); //to filter
  let result = newArray.map(val => val.map(elem => Math.pow(2,elem)));// to make the 2 powerd by the number. 
  return result;
};
...
let findMaleAndFemale = (data) => {
  ...
  }).map(element =>element.name).join(' and ');// added the spaces to make the test works in the right way 
  return mfArray;
...
let findShortest = (data) => {
  ...
  const shortes = data.reduce((shortestchar,currentchar)=>{
    if (Number (shortestchar.height) < Number (currentchar.height)){
      return shortestchar;
    }else {
      return currentchar;
    }
